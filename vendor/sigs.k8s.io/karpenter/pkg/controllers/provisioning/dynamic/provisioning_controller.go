...
func (p *ProvisioningController) Reconcile(ctx context.Context) (result reconcile.Result, err error) {
	...
	if _, err = p.provisioner.CreateNodeClaims(ctx, results.NewNodeClaims, provisioning.WithReason(metrics.ProvisionedReason), provisioning.RecordPodNomination, provisioning.IgnoreStaticNodeClaims); err != nil {
		return reconcile.Result{}, err
	}
	return reconcile.Result{RequeueAfter: singleton.RequeueImmediately}, nil
}
